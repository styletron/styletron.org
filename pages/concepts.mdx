...
`Child` is not a direct descendant of `Parent` and yet, it still gets the right value. We are crossing component boundaries without passing additional `props` or using a CSS combinator.
